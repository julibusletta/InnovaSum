                ...
                {filteredLogs.map((log) => (
                  <React.Fragment key={log._id}>
                    <tr className={`hover:bg-gray-50/30 transition-all cursor-pointer ${expandedId === log._id ? 'bg-gray-50/50' : ''}`} onClick={() => setExpandedId(expandedId === log._id ? null : log._id)}>
                      ...
                      <td className="px-6 py-5">
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-blue-50 text-blue-600 text-base font-black uppercase tracking-widest">
                          {log.service}
                        </span>
                      </td>
                      <td className="px-6 py-5">
                      ...
                      <td className="px-6 py-5">
                        {log.status ? (
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-base font-black uppercase tracking-tighter ${
                          ...
                          <span className="text-base text-gray-300 italic">No disponible</span>
                        )}
                      </td>
                          ...
                          <div className="bg-[#1e293b] rounded-lg p-5 shadow-inner">
                            ...
                            {log.headers && (
                              <div className="mt-6 pt-6 border-t border-slate-700">
                                <span className="text-base font-black text-gray-500 uppercase tracking-widest block mb-4">Headers</span>
                                <pre className="text-lg font-mono text-slate-400 overflow-x-auto whitespace-pre-wrap">
