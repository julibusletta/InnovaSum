...
export default function WholesalePage() {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `Lista_Mayorista_Innovasum_${new Date().toLocaleDateString()}.xlsx`);
  };
    ...
    <div className="wholesale-container p-4 md:p-10 bg-[#f8fafb] min-h-screen">
      ...
      <div className="no-print mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-base font-bold text-gray-400 uppercase tracking-widest">Configura márgenes por categoría y genera tu PDF</p>
        </div>
        
        <div className="flex flex-wrap items-center gap-4">
           {/* GLOBAL PRESET */}
           <div className="bg-white border border-[#e1e3e5] rounded-lg p-3 flex items-center gap-4 shadow-sm">
             <span className="text-lg font-black uppercase text-gray-400">Descuento Global:</span>
           ...
           <button 
             onClick={handlePrint}
             className="px-6 py-3 bg-[#058c8c] text-white rounded font-black text-lg uppercase tracking-widest shadow-lg hover:bg-[#047a7a] transition-all flex items-center gap-3"
           >
           ...
           <button 
             ...
             className="px-6 py-3 bg-gray-900 text-white rounded font-black text-lg uppercase tracking-widest shadow-lg hover:bg-black transition-all flex items-center gap-3"
           >
             <FaDownload /> Excel
           </button>
        </div>
      ...
      <div className="no-print mb-8 bg-white p-6 rounded-2xl shadow-sm border border-[#e1e3e5]">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            <h2 className="text-lg font-black uppercase tracking-widest text-gray-900 mb-1">Seleccionar Categorías para el PDF</h2>
            <p className="text-lg font-bold text-gray-400 uppercase tracking-tight">Selecciona qué secciones quieres incluir en el documento final</p>
          </div>
          <div className="flex items-center gap-4">
             <button 
               onClick={() => toggleAllVisibility(true)}
               className="text-lg font-black uppercase text-[#058c8c] hover:text-[#047a7a] transition-colors flex items-center gap-1"
             >
               <FaCheckSquare className="text-base" /> Seleccionar Todo
             </button>
             <button 
               onClick={() => toggleAllVisibility(false)}
               className="text-lg font-black uppercase text-gray-400 hover:text-gray-600 transition-colors flex items-center gap-1"
             >
               <FaSquare className="text-base" /> Deseleccionar Todo
             </button>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          {Object.keys(products).sort().map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-lg font-black uppercase tracking-wider transition-all border ${
      ...
      <div className="no-print mb-10">
        <input
          ...
          className="w-full max-w-md px-6 py-4 bg-white border border-[#e1e3e5] rounded-xl shadow-sm outline-none focus:border-[#058c8c] transition-all font-medium text-lg"
          value={searchTerm}
      ...
      <div className="print-only-container bg-white p-0 md:p-8 rounded-2xl shadow-sm border border-[#e1e3e5]">
        ...
        <div className="print-header hidden mb-12 items-center justify-between border-b-4 border-black pb-12">
           <div className="flex items-center gap-6">
              <img src="/images/logotest9.png" alt="Innovasum" className="h-16 w-auto object-contain" />
              <div>
                <p className="text-lg font-bold text-gray-500 uppercase tracking-[0.2em]">Lista de Precios Mayorista</p>
              </div>
           </div>
           <div className="text-right space-y-1">
              <div className="flex items-center justify-end gap-2 text-lg font-black text-black">
                <FaWhatsapp className="text-green-600" /> +54 9 11 5145-7720
              </div>
              <div className="flex items-center justify-end gap-2 text-lg font-black text-black">
                <FaInstagram className="text-pink-600" /> @innovasumarg
              </div>
              <div className="text-base font-bold text-gray-400 mt-2 italic">Fecha: {new Date().toLocaleDateString()}</div>
           </div>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-4 bg-gray-50 p-2 rounded no-print">
                   ...
                   <div className="flex items-center gap-2">
                      <span className="text-base font-black text-gray-400 uppercase tracking-tighter">Dto Cat:</span>
                      <div className="flex items-center bg-white border border-gray-200 rounded px-2">
                        <input 
                           type="number"
                           className="w-12 py-1 text-base font-black text-center outline-none bg-transparent"
                           value={config.discount}
                           onChange={(e) => updateCategoryDiscount(category, Number(e.target.value))}
                        />
                        <span className="text-lg font-bold text-[#058c8c]">%</span>
                      </div>
                   </div>
                </div>

                {/* PDF CATEGORY TITLE (Only visible in print) */}
                <h3 className="print-only-header hidden text-base font-black uppercase tracking-[0.3em] bg-black text-white px-6 py-2 mb-4 inline-block">
                  {category}
                </h3>
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="border-b-2 border-gray-200">
                        <th className="px-4 py-3 text-left text-lg font-black uppercase text-gray-400">Producto</th>
                        <th className="px-4 py-3 text-right text-lg font-black uppercase text-gray-400 w-32">P. Mayorista</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {sortedItems.map((p) => (
                        <tr key={p.id} className="hover:bg-gray-50/50 transition-colors">
                          <td className="px-4 py-3 text-base font-black text-gray-800">{p.name}</td>
                          <td className="px-4 py-3 text-right text-lg font-black text-black">
        ...
        <div className="print-footer hidden mt-12 pt-8 border-t border-gray-100 text-center">
          <p className="text-lg font-black text-gray-300 uppercase tracking-widest">
            Precios sujetos a cambios sin previo aviso • Innovasum 2026
          </p>
        </div>
      </div>
